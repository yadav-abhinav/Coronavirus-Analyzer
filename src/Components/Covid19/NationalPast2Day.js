...
function NationalPast2Day() {
  ...
  if (cases) {
    var last2Day = Number(
      cases.cases_time_series[cases.cases_time_series.length - 2].date.split(
        " "
      )[0]
    );
      ...
      >
        ...
        >
          {last2Day === today
            ...
            : last2Day === yesterday
            ? "Yesterday"
            : cases.cases_time_series[cases.cases_time_series.length - 2].date +
              " 2020"}
        </div>
        <br />
        ...
        <div className="h5 text-white">
          Tests: {cases.tested[cases.tested.length - 2].samplereportedtoday}
        </div>
      </div>
    );
  }
...
export default NationalPast2Day;
